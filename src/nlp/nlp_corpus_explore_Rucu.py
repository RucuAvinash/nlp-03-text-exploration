...
# =============================================================
# Section 2b. Modification-Declare Stop words
# =============================================================
STOP_WORDS: set[str] = {"a", "are", "the", "their", "there", "be"}
...
# ======================================================================================================
# Section 6a.Modification - Count the number of Tokens per Category
# ======================================================================================================
# Dictionary to store distinct tokens per category
category_tokens = defaultdict(set)
for token in corpus:
    category_name = token["category"]
    text = token["text"].lower()
    # Remove punctuation and split
    tokens = re.findall(r"\b\w+\b", text)
    # Add tokens to the category set
    category_tokens[category_name].update(tokens)
# Print Results
for category_name, tokens in category_tokens.items():
    print(f"Category: {category_name}")
    print(f"Distinct tokens ({len(tokens)}): {sorted(tokens)}\n")
# ===========================================================================================
# 6b.Modification- Represent distinct tokens per category in a Bar chart
# ===========================================================================================
categories = list(category_tokens.keys())
counts = [len(category_tokens[c]) for c in categories]
bar_colors = ["red", "blue", "green", "purple"]
plt.style.use("seaborn-v0_8")
plt.figure(figsize=(8, 5))
plt.bar(categories, counts, color=bar_colors)
plt.title("Distinct Token Count per Category")
plt.xlabel("Category")
plt.ylabel("Number of Distinct Tokens")
plt.tight_layout()
plt.show()
# ============================================================
